...
class LimiteConsultaEstudantes(tk.Toplevel):
    def __init__(self, controle):
        ...
        self.buttonSubmit = tk.Button(self.frameNro, text="Buscar")
...
class CtrlEstudante():
    ...
    def consultaHandler(self, event):
        ...
        for estudante in self.listaEstudantes:
            if(nroMatric == estudante.getNroMatric()):
                est = estudante

        if(est):
            str = 'Aluno encontrado!\n{} - {}\n'.format(
                est.getNroMatric(), est.getNome())
        else:
            str = 'Aluno de matrícyla {} não econtrada'.format(nroMatric)
