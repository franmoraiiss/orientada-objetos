...
class LimiteConsultaTurmas(tk.Toplevel):
    def __init__(self, controle):
        ...
        self.labelCodigo = tk.Label(
            self.frameCodigo, text="Código da Disciplina: ")
        self.labelCodigo.pack(side="left")

        self.inputCodigo = tk.Entry(self.frameCodigo, width=20)
        self.inputCodigo.pack(side="left")

        self.buttonSubmit = tk.Button(self.frameCodigo, text="Buscar")
...
class CtrlTurma():
    ...
    def consultaHandler(self, event):
        ...
        for turma in self.listaTurmas:
            if(codigo == turma.getDisciplina().getCodigo()):
                turms.append(turma)

        if(turms):
            str = 'Turma(s) encontrada(s)!\n'
            for t in turms:
                str += '{} -- {}: {}\n'.format(
                    t.getCodigo(), t.getDisciplina().getCodigo(), t.getDisciplina().getNome())
                for est in t.getEstudantes():
                    str += '{}\n'.format(est.getNome())
                str += '------\n'
        else:
            str = 'Turma da Disciplica {} não econtrada'.format(codigo)
